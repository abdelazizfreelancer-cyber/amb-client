...
async function renderMeetingsTab(){
  ...
  if(myBooking){
    const { data: slot } = await supabaseClient.from('meeting_slots').select('start_time').eq('id', myBooking.slot_id).maybeSingle();
    if(slot){ renderMeetingBooked(slot.start_time, myBooking.slot_id); return; }
  }

  const { data: slots } = await supabaseClient.from('meeting_slots').select('id, start_time').order('start_time', { ascending:true });
...
function renderMeetSlotsList(daySlots, bookedIds){
  ...
  list.innerHTML = daySlots.map(s => {
    ...
    return `<button class="meet-slot-btn" data-id="${s.id}" ${isBooked?'disabled':''}>${time} ${isBooked ? '— محجوز' : ''}</button>`;
  }).join('');
...
function renderMeetingBooked(startTime, slotId){
  const container = document.getElementById('tabContent');
  container.innerHTML = `
    <div class="meet-booked-card">
      <div class="mark">✓</div>
      <h2 style="font-family:'Cairo';color:var(--green-900);">معادك محجوز</h2>
      <p style="color:var(--ink-dim);">${new Date(startTime).toLocaleString('ar-EG')}</p>
